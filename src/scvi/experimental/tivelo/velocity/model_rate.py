...
class DNN(object):
    def __init__(self, x_u, x_s, d_nn, knn, n_dims, normalize=False, device=None):
        ...
        # initialize DNN
        self.dnn = DNNLayer(self.n_genes, n_dims).to(self.device)
    ...
    def pretrain(self, n_epochs=100, lr=1e-3, batch_size=128, loss_fun="mse", only_s=False):
        ...
        if self.normalize:
            train_data = data.TensorDataset(
                self.x_u_norm,
                self.x_s_norm,
                torch.arange(self.n_cells, device=self.device, dtype=torch.long),
            )
        ...
        for epoch in pbar:
            ...
            for u, s, idx in train_loader:
                ...
                loss.backward()
                optimizer.step()
                loss_var.append(loss.clone().detach().cpu().numpy())
    ...
    def fit(self, n_epochs=100, lr=1e-3, batch_size=64, constrain=True, loss_fun="mse", alpha_1=1, alpha_2=1,
        ...
        mse = nn.MSELoss()
        cos_sim = nn.CosineSimilarity(dim=1)
        same_c = torch.as_tensor(same_c, dtype=torch.float32, device=self.device) if same_c is not None else None
        parameter = self.dnn.parameters()
        batch_size = self.n_cells if batch_size is None else batch_size
        optimizer = optim.Adam(parameter, amsgrad=True, lr=lr)
        self.dnn.train()
        ...
        for epoch in pbar:
            ...
            for u, s, d_u, d_s, idx in train_loader:
                ...
                mse_u_.append(mse_u.clone().detach().cpu().numpy())
                mse_s_.append(mse_s.clone().detach().cpu().numpy())
                cos_u_.append(cos_u.clone().detach().cpu().numpy())
                cos_s_.append(cos_s.clone().detach().cpu().numpy())
                cos_xu_.append(cos_xu.clone().detach().cpu().numpy())
                cos_xs_.append(cos_xs.clone().detach().cpu().numpy())
        ...
        alpha = alpha.clone().detach().cpu().numpy()
        beta = beta.clone().detach().cpu().numpy()
        gamma = gamma.clone().detach().cpu().numpy()
        v_u = v_u.clone().detach().cpu().numpy()
        v_s = v_s.clone().detach().cpu().numpy()
...
def get_velocity_rate(adata, d_nn, knn, same_c=None, n_dims=[128, 64], n_epochs=100, method="simple", loss_fun="mse",
                      ...
                      batch_size=512, save_coeff=True, filter_genes=False, device=None):
    ...
    # DNN implementation
    dnn = DNN(x_u, x_s, d_nn, knn, n_dims=n_dims, normalize=normalize, device=device)
    if pretrain:
