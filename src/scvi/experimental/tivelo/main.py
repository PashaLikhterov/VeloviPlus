...
def tivelo(adata, group_key, emb_key, res=0.6, data_name="data", save_folder="results", njobs=64,
    ...
    try:
        ...
        adjusted = distances + np.identity(adata.n_obs)
        sorted_indices = np.argsort(adjusted, axis=1)
        ...
        if not rate_mode:
            adata_, v_u, v_s = get_velocity(adata, d_nn_, knn_, same_c=same_c_, n_epochs=n_epochs, loss_fun=loss_fun,
                                            ...
                                            batch_size=batch_size, filter_genes=filter_genes, device=device)
        else:
