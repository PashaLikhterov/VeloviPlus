...
def _cluster_mask(adata, cluster_key, value):
    return np.asarray(adata.obs[cluster_key].to_numpy() == value)


def keep_type(adata, nodes, target, cluster_key):
    labels = adata.obs[cluster_key].to_numpy()
    return nodes[labels[nodes] == target]
...
def cross_boundary_correctness(adata, cluster_key, velocity_key, cluster_edges, return_raw=False, x_emb="X_umap"):
    ...
    for u, v in cluster_edges:
        sel = _cluster_mask(adata, cluster_key, u)
...
def inner_cluster_coh(adata, cluster_key, velocity_key, return_raw=False):
    ...
    all_scores = {}
    for cat in clusters:
        sel = _cluster_mask(adata, cluster_key, cat)
        nbs = adata.uns['neighbors']['indices'][sel]
