...
JOB_NAME="${JOB_NAME:-velovi-dentate-uncertainty}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${WORKSPACE}/results/velovi_dentate_uncertainty}"
...
runai-bgu submit cmd \
  ...
  -- "
    python -m scvi.experimental.velovi_improvements.runner \
      ...
      --wandb-run-group \"dentate_uncertainty_weight\" \
      --plot-results
  "
