...
# Purpose: run VELOVI + (optional) transformer/TIVelo with latent-evaluation k=30
SAVE_FIGURES_LOCALLY="${SAVE_FIGURES_LOCALLY:-1}"
# so metrics/plots in the latent context use the same neighborhood size
# as the common scVelo preprocessing choice.

JOB_NAME="${JOB_NAME:-velovi-pancreas-latent30}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${WORKSPACE}/results/velovi_pancreas_latent30}"
