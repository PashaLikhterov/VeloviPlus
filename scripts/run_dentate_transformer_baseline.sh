...
JOB_NAME="${JOB_NAME:-velovi-dentate-trans-baseline}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${WORKSPACE}/results/velovi_dentate_trans_baseline}"
...
runai-bgu submit cmd \
  ...
  -- "
    python -m scvi.experimental.velovi_improvements.runner \
      ...
      --wandb-run-group \"dentate_transformer_baseline\" \
      --plot-results \
