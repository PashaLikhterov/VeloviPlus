...
JOB_NAME="${JOB_NAME:-velovi-pancreas-module}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${WORKSPACE}/results/velovi_pancreas_module}"
...
MODULE_WEIGHT="${MODULE_WEIGHT:-0.4}"
MODULE_CLUSTERS="${MODULE_CLUSTERS:-24}"
...
runai-bgu submit cmd \
  ...
  -- "
    python -m scvi.experimental.velovi_improvements.runner \
      ...
      --wandb-run-group \"pancreas_module_regularization\" \
      --plot-results
  "
