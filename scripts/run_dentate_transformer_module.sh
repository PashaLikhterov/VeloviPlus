...
JOB_NAME="${JOB_NAME:-velovi-dentate-module}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${WORKSPACE}/results/velovi_dentate_module}"
...
MODULE_WEIGHT="${MODULE_WEIGHT:-0.35}"
MODULE_CLUSTERS="${MODULE_CLUSTERS:-28}"
TRANS_ENCODER_NEIGHBORS="${TRANS_ENCODER_NEIGHBORS:-20}"
...
runai-bgu submit cmd \
  ...
  -- "
    python -m scvi.experimental.velovi_improvements.runner \
      ...
      --gene-module-weight ${MODULE_WEIGHT} \
      --gene-module-clusters ${MODULE_CLUSTERS} \
      ...
      --wandb-run-group \"dentate_module_regularization\" \
      --plot-results
  "
