...
JOB_NAME="${JOB_NAME:-velovi-pancreas-transenc}"
...
OUTPUT_DIR="${OUTPUT_DIR:-${WORKSPACE}/results/velovi_pancreas_transenc}"
...
TRANSFORMER_ENCODER_NEIGHBORS="${TRANSFORMER_ENCODER_NEIGHBORS:-16}"
TRANSFORMER_ENCODER_NEIGHBOR_WEIGHT="${TRANSFORMER_ENCODER_NEIGHBOR_WEIGHT:-0.0}"
...
runai-bgu submit cmd \
  ...
  -- "
    python -m scvi.experimental.velovi_improvements.runner \
      ...
      --transformer-encoder-max-neighbors ${TRANSFORMER_ENCODER_NEIGHBORS} \
      --transformer-encoder-neighbor-weight ${TRANSFORMER_ENCODER_NEIGHBOR_WEIGHT} \
      --gnn-hidden-dim 128 \
      --gnn-dropout 0.1 \
      --num-workers 0 \
      ...
      --wandb-run-group \"pancreas_transformer_encoder\" \
      --plot-results \
